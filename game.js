const Game = (() => {
  ...
  function kaufen(w){ const p=Economy.getPreis(w); if(state.gold>=p){ state.gold-=p; state.inventar[w]++; UI.log(`Du kaufst 1 ${w} für ${p} Gold`); Animations.animateCoinForRival(w,"Spieler"); tick(); }else UI.log("Nicht genug Gold!"); }
  function verkaufen(w){ if(state.inventar[w]>0){ const p=Economy.getPreis(w); state.inventar[w]--; state.gold+=p; UI.log(`Du verkaufst 1 ${w} für ${p} Gold`); Animations.animateCoinForRival(w,"Spieler"); tick(); }else UI.log(`Keine ${w} zum verkaufen!`); }
